...
if args.version == 'a':
    ...
    # Second Version
    # Test accuracy(after Pruning): 0.91625
    # Compressed size: 38347
    mfcc = True
    alpha = 0.26
    num_filter = 512
    momentom = 0.9
    STFT_OPTIONS = {'frame_length': 650, 'frame_step': 350, 'mfcc': False}
    MFCC_OPTIONS = {'frame_length': 650, 'frame_step': 350, 'mfcc': True,
                    'lower_frequency': 20, 'upper_frequency': 4000, 'num_mel_bins': 30,
                    'num_coefficients': 10}
    epoch_num = 20
elif args.version == 'c':
    # Test accuracy(after Pruning): 0.91125
    # Compressed size: 16628
    ...
    momentom = 0.9
    STFT_OPTIONS = {'frame_length': 650, 'frame_step': 350, 'mfcc': False}
    MFCC_OPTIONS = {'frame_length': 512, 'frame_step': 322, 'mfcc': True,
                    'lower_frequency': 20, 'upper_frequency': 4000, 'num_mel_bins': 30,
                    'num_coefficients': 10}
    epoch_num = 25
...
def ds_cnn_model(num_filter, momentom, alpha=1):
    ds_cnn = keras.Sequential()
    ds_cnn.add(
        layers.Conv2D(filters=num_filter * alpha, kernel_size=[3, 3], strides=strides, use_bias=False,
                      activation='relu'))
    ds_cnn.add(layers.BatchNormalization(momentum=momentom))
    # ds_cnn.add(layers.DepthwiseConv2D(kernel_size=[3, 3], strides=[1, 1], use_bias=False))
    # ds_cnn.add(
    #     layers.Conv2D(filters=num_filter * alpha, kernel_size=[1, 1], strides=[1, 1], use_bias=False,
    #                   activation='relu'))
    # ds_cnn.add(layers.BatchNormalization(momentum=momentom))
    ds_cnn.add(layers.DepthwiseConv2D(kernel_size=[3, 3], strides=[1, 1], use_bias=False))
    ds_cnn.add(
    ...
    ds_cnn.compile(loss=SparseCategoricalCrossentropy(from_logits=True), optimizer='adam', metrics=['accuracy'])
    return ds_cnn
...
if args.version == 'a':
    ...
    model = ds_cnn_model(num_filter=num_filter, momentom=momentom, alpha=alpha)
elif args.version == 'c':
    model = cnn_model(num_filter=num_filter, momentom=momentom, alpha=alpha)

# units = 8
model.fit(train_ds, verbose=1, epochs=epoch_num, batch_size=32)
